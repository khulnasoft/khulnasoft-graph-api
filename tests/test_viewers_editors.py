...
def test_is_viewer(mocker):
  ...
  assert khulnasoft_graph_api.VTGraph.is_viewer(VT_USER, GRAPH_ID, API_KEY)
  mocker.resetall()
...
def test_is_not_viewer(mocker):
  ...
  mocker.patch("requests.get", return_value=m)
  assert not khulnasoft_graph_api.VTGraph.is_viewer(VT_USER, GRAPH_ID, API_KEY)
  mocker.resetall()


def test_viewer_load_error(mocker):
  mocker.patch("requests.get", return_value=mocker.Mock(status_code=401))
  with pytest.raises(khulnasoft_graph_api.errors.LoadError):
    khulnasoft_graph_api.VTGraph.is_viewer(VT_USER, GRAPH_ID, API_KEY)
  mocker.resetall()
...
def test_is_editor(mocker):
  m = mocker.Mock(status_code=200, json=mocker.Mock(return_value=API_RESPONSE))
  mocker.patch("requests.get", return_value=m)
  assert khulnasoft_graph_api.VTGraph.is_editor(VT_USER, GRAPH_ID, API_KEY)
  mocker.resetall()
...
def test_is_not_editor(mocker):
  m = mocker.Mock(status_code=200, json=mocker.Mock(return_value={"data": []}))
  mocker.patch("requests.get", return_value=m)
  assert not khulnasoft_graph_api.VTGraph.is_editor(VT_USER, GRAPH_ID, API_KEY)
  mocker.resetall()
...
def test_editor_load_error(mocker):
  mocker.patch("requests.get", return_value=mocker.Mock(status_code=401))
  with pytest.raises(khulnasoft_graph_api.errors.LoadError):
    khulnasoft_graph_api.VTGraph.is_editor(VT_USER, GRAPH_ID, API_KEY)
  mocker.resetall()
