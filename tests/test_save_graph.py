...
def test_save_graph_collaborator_not_found(mocker):
  ...
  with pytest.raises(khulnasoft_graph_api.errors.CollaboratorNotFoundError):
...
def test_save_graph_error(mocker):
  ...
  with pytest.raises(khulnasoft_graph_api.errors.SaveGraphError):
...
def test_get_link():
  ...
  assert (test_graph.get_ui_link() ==
          "https://www.khulnasoft.com/graph/%s" % graph_id)
