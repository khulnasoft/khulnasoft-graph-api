...
def test_search_connection_first_level(mocker):
  ...
  node_a = khulnasoft_graph_api.Node(
      "26c808a1eb3eaa7bb29ec2ab834559f06f2636b87d5f542223426d6f238ff906",
      "file")
  node_b = khulnasoft_graph_api.Node(
...
def test_search_connection_second_level(mocker):
  ...
  side_effects = list(
      request_response_first_level *
      len(khulnasoft_graph_api.Node.NODE_EXPANSIONS["file"]))
  side_effects += (
      request_response_second_level *
      len(khulnasoft_graph_api.Node.NODE_EXPANSIONS["file"]))
  node_a = khulnasoft_graph_api.Node(
      "26c808a1eb3eaa7bb29ec2ab834559f06f2636b87d5f542223426d6f238ff906",
      "file")
  node_b = khulnasoft_graph_api.Node("nsis.sf.net", "domain")
...
def test_search_connection_third_level(mocker):
  ...
  side_effects += (
      request_response_third_level *
      len(khulnasoft_graph_api.Node.NODE_EXPANSIONS["domain"])
  )
  node_a = khulnasoft_graph_api.Node(
      "26c808a1eb3eaa7bb29ec2ab834559f06f2636b87d5f542223426d6f238ff906",
      "file")
...
def mock_request(url, headers, timeout):
  ...
  assert timeout == khulnasoft_graph_api.VTGraph.REQUEST_TIMEOUT
...
def test_search_connection_second_level_real_data(mocker):
  ...
  intermediate_node = khulnasoft_graph_api.Node(
      INTERMEDIATE_NODE_ID,
      "file"
  )
  node_b = khulnasoft_graph_api.Node(
  ...
  for node_type in six.itervalues(EXPANSION_NODES):
    total_expansion_calls += len(khulnasoft_graph_api.Node.NODE_EXPANSIONS[node_type])
