...
def create_dummy_graph():
  return khulnasoft_graph_api.VTGraph(
      "Dummy api key", verbose=False, private=False, name="Graph test",
      user_editors=["dummy_user"], group_viewers=["khulnasoft"])
...
def test_create_empty_group():
  ...
  with pytest.raises(khulnasoft_graph_api.errors.CreateGroupError,
...
def test_create_group_with_nodes_already_grouped():
  ...
  test_graph.add_node("khulnasoft.com", "domain")
  test_graph.add_node("google.com", "domain")
  test_graph.create_group(['khulnasoft.com', 'google.com'], 'Group 1')
