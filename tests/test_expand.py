...
def test_expansion_existing_node(mocker):
  ...
  expansion_node = khulnasoft_graph_api.Node(
...
def test_expand_not_existing_node():
  ...
  with pytest.raises(khulnasoft_graph_api.errors.NodeNotFoundError,
...
def test_not_supported_expansion(mocker):
  ...
  with pytest.raises(khulnasoft_graph_api.errors.NodeNotSupportedExpansionError,
