...
import khulnasoft_graph_api.graph

with (
    open(os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "resources/khulnasoft_graph_id.json"))) as fp:
...
def test_load_graph_with_match(mocker):
  ...
  test_graph = khulnasoft_graph_api.graph.VTGraph.load_graph(GRAPH_ID, API_KEY)
  nodes = [
  ...
  assert "khulnasoft" in test_graph.group_editors
...
def test_load_graph_without_editors_and_viewers(mocker):
  ...
  for source, target, connection_type in links:
    assert test_graph.links[(source, target, connection_type)]
  assert "khulnasoft" not in test_graph.group_editors
...
def test_load_graph_with_fail_request(mocker):
  ...
  with pytest.raises(
      khulnasoft_graph_api.errors.LoadError,
    ...
    khulnasoft_graph_api.graph.VTGraph.load_graph(GRAPH_ID, API_KEY)
  mocker.resetall()
...
def test_load_graph_wrong_json(mocker):
  ...
  with pytest.raises(
      khulnasoft_graph_api.errors.InvalidJSONError):
    side_effects = [
        GRAPH_WRONG_RESPONSE_DATA,
        VIEWERS_RESPONSE_DATA,
        EDITORS_RESPONSE_DATA
